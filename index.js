class Play {
  ...
  preload() {
    this.load.image( 'hero', 'images/leanixlogo.jpeg'); // 'https://s3-us-west-2.amazonaws.com/s.cdpn.io/836/dude.png' );
  ...
  heroCreate() {
    this.velocity = 350;
    // basic hero setup
    this.hero = game.add.sprite( this.world.centerX, this.world.height - 35, 'hero' );
  ...
  scoreCreate() {
    ...
    this.levelCounter = 0;
    this.scoreText = this.add.text(0, 0, `${this.score} points`, { fontFamily: '"Roboto Condensed"' });
  }
  ...
  heroMove() {
    ...
    if (this.hero.body.touching.down && !this.oldBodyDown && !this.isStarting) {
      this.levelCounter++;
      this.score += 100 * (this.scoreMultiplicator + 1);

      if (this.levelCounter % 10 === 0) {
    ...
    // handle hero jumping
    if( (this.cursor.up.isDown || !this.isStarting) && this.hero.body.touching.down ) {
    ...
    this.oldBodyDown = this.hero.body.touching.down;
  }
}
